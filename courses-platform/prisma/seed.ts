...
import { DEFAULT_CATEGORIES, COURSE_LEVELS, COURSE_TYPES } from "../src/lib/constants";
...
type CourseSeed = {
  ...
  durationText: string;
  totalSeats: number;
  fillerEnrollCount: number; // عدد المتعلمين التجريبيين الذين يُسجَّلون مسبقًا
...
// عناوين الدورات التي سيُسجَّل بها المستخدم التجريبي (لتغطية الحالات الثلاث في "دوراتي")
...
async function main() {
  ...
  console.log(`تم إنشاء ${createdCount} دورة، و${DEFAULT_CATEGORIES.length} تصنيفات.`);
  console.log(`تم إنشاء ${enrollmentCount} تسجيلًا تجريبيًا.`);
  console.log("---");
  console.log("حساب المسؤول: admin@example.com / Admin@12345");
  console.log("حساب مستخدم تجريبي: user@example.com / User@12345");
}
